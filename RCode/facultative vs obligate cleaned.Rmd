...
coev<-read.csv("~/OneDrive - University of Massachusetts Boston/Yeast/Coevolution no cheater/diff_time-shift_facult2021.csv")

coevb<-read.csv("~/OneDrive - University of Massachusetts Boston/Yeast/Coevolution no cheater/diff_time-shift_fac+ob2021.csv")
...
#Trying out Sheng's code to calculate the difference from ancestrals

coev2=coev%>%
  mutate(is_anc=if_else(Culture_ID=='Ancestral',1,0))%>%
  group_by(Combo, Experimental_plate) %>%
  mutate(ODc_anc=sum(is_anc*OD)/sum(is_anc),
                     ODcc=(OD-ODc_anc)/ODc_anc, Combo=as.character(Combo))%>%
           ungroup()
...
ggplot(coev2, aes(time_point, ODcc, color=Culture_ID))+
  facet_grid(type~Combo)+
  geom_point()+
  geom_line(data=coev2%>%group_by(Culture_ID, time_point)%>%summarise(ODcc=mean(ODcc)))

#6C from plate 2 @ week 15 seems to be growing more than the others for all combinations, but it was intermediary at other time-points.
#5D plate 1 grows more than others @ weeks 8 and 10, for all combinations.
...
#Testing if there is significant differences among cultures
mod_EA<-mixed(ODcc~Culture_ID*time_point+(1|Experimental_plate), data=coev2%>%filter(type=="EA"))
anova(mod_EA)
...
mod_AE<-mixed(ODcc~Culture_ID*time_point+(1|Experimental_plate), data=coev2%>%filter(type=="AE"))
anova(mod_AE)
...
#Only marginal differences, but interaction is significant. Main differences are in week 15, between 6H, 5A, 5D in comparison to 6C.

mod_EE<-mixed(ODcc~Culture_ID*time_point+(1|Experimental_plate), data=coev2%>%filter(type=="EE"))
anova(mod_EE)
...
#5D and 6C growing more when it's both evolved.

str(coev2)

coev2$time_point<-as.numeric(coev2$time_point)

dataop_long_sum = 
  coev2 %>% 
  group_by(Combo, Culture_ID, time_point, type) %>% 
...
test_LysE<-lmer(ODcc~time_point-1 + (1|Culture_ID), data=coev2%>%filter(type=="AE" & Culture_ID != 'Ancestral'))
summary(test_LysE)
...
test_AdeE<-lmer(ODcc~time_point-1 + (1|Culture_ID), data=coev2%>%filter(type=="EA" & Culture_ID != 'Ancestral'))
summary(test_AdeE)
Anova(test_AdeE)
check_model(test_AdeE)

test_EE<-lmer(ODcc~time_point-1 + (1|Culture_ID), data=coev2%>%filter(type=="EE" & Culture_ID != 'Ancestral'))
summary(test_EE)
Anova(test_EE)
check_model(test_EE)
...
coev2b=coevb%>%
  mutate(is_anc=if_else(type=='AA',1,0))%>%
  group_by(Combo, Experimental_plate) %>%
  mutate(ODc_anc=sum(is_anc*OD)/sum(is_anc),
                     ODcc=(OD-ODc_anc)/ODc_anc, Combo=as.character(Combo))%>%
           ungroup()

coev2b$time_point<-as.factor(coev2b$time_point)

#For Ade anc, Lys Evo
AEobfac<-lmer(ODcc~time_point*Mutualism + (1|Combo), data=coev2b%>%filter(type=="AE"))
anova(AEobfac)
summary(AEobfac)

mod_means<-emmeans(AEobfac, list(pairwise ~ time_point*Mutualism))
...
#For Ade evo, Lys anc
EAobfac<-lmer(ODcc~time_point*Mutualism + (1|Combo), data=coev2b%>%filter(type=="EA"))
summary(EAobfac)
anova(EAobfac)

mod_means2<-emmeans(EAobfac, list(pairwise ~ time_point*Mutualism))

model_means_cld <- cld(object = mod_means2,
                       adjust = "Tukey",
                       Letters = letters,
                       alpha = 0.05)
...
#For Ade evo, Lys evo
EEobfac<-lmer(ODcc~time_point*Mutualism + (1|Combo), data=coev2b%>%filter(type=="EE"))
summary(EEobfac)
anova(EEobfac)

mod_means3<-emmeans(EEobfac, list(pairwise ~ time_point*Mutualism))

model_means_cld <- cld(object = mod_means3,
                       adjust = "Tukey",
                       Letters = letters,
                       alpha = 0.05)
...
head(eff_both2)
eff_both2$time_point<-as.factor(eff_both2$time_point)

ggplot(eff_both2, aes(time_point, ODcc, color=Culture_ID))+
  facet_grid(Mutualism~Strain)+
  geom_point()+
  geom_line(data=coev2%>%group_by(Culture_ID, time_point)%>%summarise(ODcc=mean(ODcc)))


#I'll remove the extreme values (2 points 1039 week 8,culture 47 obligate mutualism) . I don't know what is the deal with these points, I double-checked them...
problem_index<-which(eff_both2$ODcc >1) #Specify the exact point above that point
eff_both2[problem_index, ] 
eff_both2.2<-eff_both2[-problem_index, ]

#Plot again
ggplot(eff_both2.2, aes(time_point, ODcc, color=Culture_ID))+
  facet_grid(Mutualism~Strain)+
  geom_point()+
  geom_line(data=coev2%>%group_by(Culture_ID, time_point)%>%summarise(ODcc=mean(ODcc)))
...
eff_LysOP<-eff_both2.2[grep("LysOP", eff_both2.2$Mutualist),]
...
```{r}
write.csv(prod_both_Lys, "prod_both_lys.csv")

write.csv(prod_both_ade, "prod_both_ade.csv")

prod_fac<-read.csv("Facult_prod.csv")

prod_fac$time_point<-as.factor(prod_fac$time_point)
prod_fac$time_point<-as.numeric(prod_fac$time_point)

dataeff_long_sum = 
  prod_fac %>% 
  group_by(Mutualist, Culture_ID, time_point, Strain) %>% 
          summarise(testODcc = mean(testODcc))  %>% 
  filter(Culture_ID != "Ancestral") %>% 
  mutate(testODcc = if_else(time_point == 0, 0, testODcc))

```


```{r}
#Correcting overproduction by growth - The corrected datasheet is already included
...
```{r}
#Getting corrected production data for obligate mutualism
#Density dependent growth
calc_Nr = function(t, r, K, N0 = 0.1, tdelay = 0){
  ex_part = exp(-r*(t-tdelay))
  Nt = K/(1+((K-N0)/N0)*ex_part)
...
#Use the integral to analyze my data for AdeOP

#prod_Ade_ob<-read.csv("prod_obligAde_July20v2.csv")
#head(prod_Ade_ob)

#growth_int = 1:length(prod_Ade_ob$OD)
#for(i in 1:length(prod_Ade_ob$OD)){
#  growth_int[i] = get_growth_int(prod_Ade_ob$OD[i], ade = (prod_Ade_ob$Mutualist[i] == "AdeOP"), t_od = 48)
#}
#prod_Ade_ob$growth_int = growth_int
#data_mayra_Ade_ob = 
#  prod_Ade_ob %>% 
#  mutate(corrected_OD = OD_teststrain/growth_int,
#         OD_scaled = scale(OD_teststrain),
...
#write.table(data_mayra_Ade_ob, "corrected_prod_Ade_ob.txt", sep="\t")


#Use ratio to analyze data for LysOP
#prod_Lys_ob<-read.csv("production_Lys_obligate.csv")
#head(prod_Ade_ob)
...
#write.table(data_mayra_Lys_ob, "corrected_prod_Lys_ob.txt", sep="\t")
...
test_prodAde<-lmer(testODcc~time_point-1 +(1|Culture_ID), data=prod_both_Ade%>%filter(Mutualism=="facultative" & !Culture_ID=="Ancestral"))
